...
{
	u32 tempreg=0;
	RCC->AHB2ENR|=1<<0;    		//使能PORTB时钟	 
  RCC->AHB2ENR|=1<<1;				//使能PORTD时钟	 
	RCC->AHB2ENR|=1<<2;    		//使能PORTE时钟   
	RCC->AHB3ENR|=1<<8;   		//QSPI时钟使能

	GPIO_Set(GPIOA,1<<6,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PB2复用功能输出	
	GPIO_Set(GPIOA,1<<7,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PB6复用功能输出
	
	GPIO_Set(GPIOB,0X1<<0,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PD11复用功能输出
	GPIO_Set(GPIOB,0X1<<1,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PD12复用功能输出	
	GPIO_Set(GPIOB,0X1<<10,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PD13复用功能输出	
	GPIO_Set(GPIOB,0X1<<11,GPIO_MODE_AF,GPIO_OTYPE_PP,GPIO_SPEED_HIGH,GPIO_PUPD_PU);	//PE2复用功能输出	
  ...
  GPIO_AF_Set(GPIOA,6,10);		//PB2,AF9
	...
	RCC->AHB3RSTR|=1<<8;		//复位QSPI
	RCC->AHB3RSTR&=~(1<<8);	//停止复位QSPI
