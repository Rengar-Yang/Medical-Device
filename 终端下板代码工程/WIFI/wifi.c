...
char cmd_state(unsigned char time,unsigned char n,char* str)
{
	...
	else if(CMD_S[n]!=0&&CMD_S[_next]==0)  //这条指令执行完并且下一条指令没有执行才可以
	{
//		if(Usart2_RxCompleted==1)    //如果Usart2_RxCompleted等于1，表示接收数据完成
//		{
			Usart2_RxCompleted=0;
			if(strstr(Usart2_RxBuff,str)) //比对字符
			{
				time_cnt=0;
				CMD_S[n]=2;
//				console_log(1,"%s",&Data_buff[2]);
			...
			{
				time_cnt=0;
				CMD_S[n]=3;  //因为和服务器通信不会立刻返回信息，所以要一直等待到超市
//				console_log(1,"%s",&Data_buff[2]);
				return 3; //错误
			}
//		}
...
//	console_log(1,"-----------------\n");
	time_str_buf[Find_string(&Data_buff[2],"Date:", "GMT",time_str_buf)]='\0';
//	console_log(1,"%s",&str_buf[0]);
//	console_log(500," ");
//	
	timeDate_Set.date.date = (time_str_buf[6]-'0')*10+ (time_str_buf[7]-'0');
	timeDate_Set.date.month = (month_t)((time_str_buf[5]-'0')*10+ (time_str_buf[6]-'0')-1); //注意需要减去1
	timeDate_Set.date.year = (time_str_buf[15]-'0')*10+ (time_str_buf[16]-'0');
	
	
	timeDate_Set.time.hour = (time_str_buf[18]-'0')*10+ (time_str_buf[19]-'0')+8;
	timeDate_Set.time.mins = (time_str_buf[21]-'0')*10+ (time_str_buf[22]-'0');
	timeDate_Set.time.secs = (time_str_buf[24]-'0')*10+ (time_str_buf[25]-'0');
...
//		console_log(500,"AT+CIPMODE=1");
	...
	while(cmd_state(100,1,"OK")!=2)
		WiFi_send("AT+CIPMODE=1\r\n");
		
	AT_state=cmd_state(100,2,"OK");
	if(AT_state==0)
	{
//		console_log(500,"AT+CIPMUX=0");
		WiFi_send("AT+CIPMUX=0\r\n");
	}
	while(cmd_state(100,2,"OK")!=2)
		WiFi_send("AT+CIPMUX=0\r\n");
	
	AT_state=cmd_state(100,3,"OK");
	if(AT_state==0)  //因为和服务器建立连接
	{
//		console_log(500,"Connecting TCP...");
		WiFi_send("AT+CIPSTART=\"TCP\",\"%s\",%d\r\n",ServerIP3,ServerPort);//发送连接服务器指令
	}
	while(cmd_state(100,3,"OK")!=2)
		WiFi_send("AT+CIPSTART=\"TCP\",\"%s\",%d\r\n",ServerIP3,ServerPort);//发送连接服务器指令;
	
	AT_state=cmd_state(100,4,"OK");
	if(AT_state==0)
	{
//		console_log(500,"AT+CIPSEND");
		WiFi_send("AT+CIPSEND\r\n");                  //发送进入透传指令
	}
//	while(cmd_state(100,4,"OK")!=2)
//		WiFi_send("AT+CIPSEND\r\n");
	
	AT_state=cmd_state(100,5,"Date");
	if(AT_state==0)
	{
//		console_log(500,"Updata Time");
		WiFi_send("GET http://cgi.im.qq.com\r\n");
	}
	while(cmd_state(100,5,"Date")!=2)
		WiFi_send("GET http://cgi.im.qq.com\r\n");
...
void get_weather_cmd(void)
{
	...
	if(cmd_state(10,5,"CN")==0)
	{
		...
		WiFi_send("GET https://api.seniverse.com/v3/weather/now.json?key=SIN2EqCTJrkaqqOpk&location=xuzhou&language=en&unit=c\r\n");
	}
	else if(CMD_S[5]==2)
	{
		CMD_S[5]='o';
